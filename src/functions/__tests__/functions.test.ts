...
describe('DeliveryFeeCalculator', () => {
    test('when distance is less than or equal to 1000m, the minimal delivery fee is 2€', () => {
        const result = calculateDeliveryPrice({...baseCart, deliveryDistance: 1000})
        expect(result).toBe(2)
        expect(result).toBe(minimalDeliveyFee)
    })

    test('when distance is more than 1000m, add 1€ per each next 500m', () => {
        let result = calculateDeliveryPrice({...baseCart, deliveryDistance: 1001})
    ...
    test('when order date is during the Friday rush, 3-7pm, delivery fee * 1.2', () => {
        let result = calculateDeliveryPrice({...baseCart, orderDate: new Date('2024-01-22T09:07')})
        expect(result).toBe(minimalDeliveyFee)

        result = calculateDeliveryPrice({...baseCart, orderDate: new Date('2024-01-22T17:00')})
        expect(result).toBe(minimalDeliveyFee)

        result = calculateDeliveryPrice({...baseCart, orderDate: new Date('2024-01-26T14:59')})
        expect(result).toBe(minimalDeliveyFee)
        
        result = calculateDeliveryPrice({...baseCart, orderDate: new Date('2024-01-26T19:01')})
        expect(result).toBe(minimalDeliveyFee)

        result = calculateDeliveryPrice({...baseCart, orderDate: new Date('2024-01-26T15:00')})
        expect(result).toBe(minimalDeliveyFee * 1.2)

        result = calculateDeliveryPrice({...baseCart, orderDate: new Date('2024-01-26T18:59')})
        expect(result).toBe(minimalDeliveyFee * 1.2)
    })

    test('when delivery fee is more than 15€, the delivery fee is free (0€)', () => {
        const result = calculateDeliveryPrice({...baseCart, cartValue: 1, deliveryDistance: 6501, numberOfItems: 14, orderDate: new Date('2024-01-26T18:59')})
